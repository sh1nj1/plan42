module ApplicationHelper
  ...
  # `lang` carries the code-fence language on <pre lang="…">. The rendered body
  # child-row description goes through this sanitizer (tree_builder), so dropping
  # `lang` here makes the body row lose the language and content-detect it — out
  # of step with the editor/heading, which keep it. Keep it so every surface
  # honors the one explicit language.
  EMBED_ALLOWED_ATTRS = %w[src title frameborder allow allowfullscreen style href class alt width height target rel sgid content-type filename colspan rowspan controls preload poster lang].freeze
