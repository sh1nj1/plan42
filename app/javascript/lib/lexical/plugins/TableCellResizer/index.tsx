...
function TableCellResizer({ editor }: { editor: LexicalEditor }): JSX.Element {
  ...
  const resetState = useCallback(() => {
    ...
    tableColumnWidthsRef.current = null;
  }, []);
  ...
  const tableColumnWidthsRef = useRef<number[] | null>(null);
    ...
    (widthChange: number) => {
      ...
      editor.update(
        () => {
          ...
          // Initialize baseline widths from DOM if not yet captured for this drag session
          if (!tableColumnWidthsRef.current) {
            const colCount = tableNode.getColumnCount();
            const storedWidths = tableNode.getColWidths() || Array(colCount).fill(MIN_COLUMN_WIDTH);
            const captured = new Array(colCount).fill(0);

            for (let c = 0; c < colCount; c++) {
              let cell: TableCellNode | null = null;
              // Find a cell starting at column c
              for (let r = 0; r < tableMap.length; r++) {
                const mapEntry = tableMap[r][c];
                if (mapEntry.startRow === r && mapEntry.startColumn === c) {
                  cell = mapEntry.cell;
                  break;
                }
              }
              ...
              if (cell) {
                const elem = editor.getElementByKey(cell.getKey());
                if (elem) {
                  const rect = elem.getBoundingClientRect();
                  const span = cell.getColSpan();
                  const totalStored = storedWidths.slice(c, c + span).reduce((a, b) => a + b, 0);

                  for (let k = 0; k < span; k++) {
                    const ratio = totalStored > 0 ? storedWidths[c + k] / totalStored : 1 / span;
                    captured[c + k] = rect.width * ratio;
                  }
              ...
              // Fallback
              captured[c] = storedWidths[c];
            }
            tableColumnWidthsRef.current = captured;
          }

          const baselineWidths = tableColumnWidthsRef.current;
          if (!baselineWidths) return;

          const newColWidths = [...baselineWidths];
          const newWidth = Math.max(baselineWidths[columnIndex] + widthChange, MIN_COLUMN_WIDTH);
