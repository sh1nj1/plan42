...
module CollavreOpenclaw
  class PendingCallbackTest < ActiveSupport::TestCase
    ...
    test "nonce is unique" do
      pending1 = PendingCallback.create_for_request(user: @user, creative_id: 1)
      pending2 = PendingCallback.create_for_request(user: @user, creative_id: 2)
    ...
    test "verify_and_consume! returns nil for expired nonce" do
      pending = PendingCallback.create_for_request(user: @user, creative_id: 1)
    ...
    test "cleanup_expired! removes old callbacks" do
      valid = PendingCallback.create_for_request(user: @user, creative_id: 1)
      expired = PendingCallback.create_for_request(user: @user, creative_id: 2)
