...
class SessionTimezoneTest < ActionDispatch::IntegrationTest
  setup do
    @user = User.create!(email: "user@example.com", password: TEST_PASSWORD, name: "User", email_verified_at: Time.current)
  end

  test "stores timezone on login" do
    post session_path, params: { email: @user.email, password: TEST_PASSWORD, timezone: "Asia/Tokyo" }
    assert_redirected_to root_url
    assert_equal "Asia/Tokyo", @user.reload.timezone
  end

  test "allows clearing timezone from profile" do
    post session_path, params: { email: @user.email, password: TEST_PASSWORD }
