...
module Collavre
  module Orchestration
    class StuckDetectorTest < ActiveSupport::TestCase
      ...
      test "detects stuck pending_approval task" do
        policy = create_policy_with_stuck_detection(enabled: true, task_threshold: 30)
        policy.update!(config: policy.config.merge("approval_timeout_minutes" => 60))
        ...
        task.update_columns(created_at: 2.hours.ago, updated_at: 2.hours.ago)

        detector = StuckDetector.new
        stuck_items = detector.detect

        approval_stuck = stuck_items.find { |item| item.type == :task && item.item.id == task.id }
        assert_not_nil approval_stuck
        assert_equal :approval_timeout, approval_stuck.reason
      ensure
        policy&.destroy
      end

      test "auto-recovers pending_approval tasks by marking them as cancelled" do
        policy = create_policy_with_stuck_detection(enabled: true, task_threshold: 30)
        policy.update!(config: policy.config.merge("approval_timeout_minutes" => 60))
      ...
      test "does not detect recent pending_approval task as stuck" do
        policy = create_policy_with_stuck_detection(enabled: true, task_threshold: 30)
        policy.update!(config: policy.config.merge("approval_timeout_minutes" => 60))

        task = Collavre::Task.create!(
          name: "Recent approval task",
          agent: @ai_agent,
          status: "pending_approval",
          trigger_event_payload: { "creative" => { "id" => @creative.id } },
          topic_id: @topic.id
        )
        # Task was created just now, not stuck yet

        detector = StuckDetector.new
        stuck_items = detector.detect

        approval_stuck = stuck_items.find { |item| item.type == :task && item.item.id == task.id }
        assert_nil approval_stuck
      ensure
        policy&.destroy
      end
