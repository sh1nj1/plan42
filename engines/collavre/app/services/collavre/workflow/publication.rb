    ...
    class Publication
      ...
      def reason
        ...
        safety.reason || EnvelopeValidation.reason(@row.context) || identity_reason ||
          safety.comment_reason(Comment.find_by(id: @row.context.dig("comment", "id")), topic_id: output_topic_id) || replies_reason
      end
      ...
      def identity_reason
        ...
        return "scope_changed" unless @row.context.dig("creative", "id") == @execution.chain.creative_id && @row.context.dig("topic", "id").to_i == output_topic_id
        @execution.chain.depth_reason(event["depth"])
      end

      def output_topic_id
        (@execution.context["invocation"] || @execution.context).dig("topic", "id").to_i
      end
