...
export function createDragDropRegistry({
  ...
  const resolveZone = (event) => {
    const kind = getKind(event.dataTransfer)
    const zone = zones.find((candidate) => {
      const element = matchingElement(root, event, candidate.selector)
      return element && acceptsKind(candidate.accepts, kind, event)
    })
    if (!zone) return null

    const element = matchingElement(root, event, zone.selector)
    const hit = zone.hitTest ? zone.hitTest({ el: element, event, kind }) : true
