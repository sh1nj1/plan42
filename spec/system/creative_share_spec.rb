...
RSpec.describe 'Creative 공유 리스트', type: :system do
  let!(:user) { User.create!(email_address: 'user1@example.com', password: 'password') }
  ...
  it '공유 리스트가 정상적으로 표시된다' do
    ...
    fill_in placeholder: I18n.t('users.new.enter_your_email_address'), with: user.email_address
    ...
    expect(page).to have_content('user1@example.com - Permission Read')
  end
end
